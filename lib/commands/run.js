...
function run(scriptPath, options, cb) {
  ...
  async.waterfall(
    ...
    function done(err, script) {
      if (err) {
        ...
        cb && typeof cb === 'function' ? cb(err) : process.exit(1);
      }
        ...
        if (script.config.ensure) {
          ...
          _.each(script.config.ensure, function(max, k) {
            ...
            if (latency[bucket]) {
              if (latency[bucket] > max) {
                ...
                cb && typeof cb === 'function' ? cb(`ensure condition failed: ensure.${bucket} < ${max}`) : process.exit(1);
              }
            }
          });
          ...
          if (typeof script.config.ensure.maxErrorRate !== 'undefined') {
            ...
            if (failRate > script.config.ensure.maxErrorRate) {
              ...
              cb && typeof cb === 'function' ? cb(`ensure condition failed: ensure.maxErrorRate <= ${script.config.ensure.maxErrorRate}`) : process.exit(1);
            }
          }
        }
      ...
      function gracefulShutdown() {
        ...
        runner.shutdown(function() {
          async.eachSeries(
            ...
            function done() {
              debug('All done');
              cb && typeof cb === 'function' ? cb() : process.exit(0);
            }
          );
        });
      }
